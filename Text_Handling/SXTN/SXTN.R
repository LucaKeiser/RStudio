...
lyrics <- readtext::readtext("Text_Handling/SXTN/Lyrics/")
...
lyrics_tokens <- lyrics_tokens %>% 
  tokens_remove(pattern = word(c(stopwords("de"),
                                 "ne", "n", "mal", "is", "ab", "für'n", "schon", "nen", "mehr",
                                 "ma", "gibt", "grad", "dass")), valuetype = 'fixed')
...
# filter out the sxtn-feature
lyrics_dfm_wordstem_filtered <- quanteda::dfm_select(lyrics_dfm_wordstem, pattern = "sxtn",
                                                     selection = "remove")

lyrics_dfm_wordstem_filtered

set.seed(12345)
quanteda.textplots::textplot_wordcloud(lyrics_dfm_wordstem_filtered,
                                       max_words = 80,
                                       min_size = 2,
                                       max_size = 6)
