...
lyrics <- readtext::readtext("Text_Handling/Limp_Bizkit/Lyrics/")
...
lyrics_tokens <- lyrics_tokens %>% 
  tokens_remove(pattern = word(c(stopwords("en"), "i'ma")), valuetype = 'fixed')
...
freq %>% 
  ...
  labs(
    ...
    subtitle = "Most common words in the Greatest Hitz album",
    x = "",
    y = "Count"
  ) +
