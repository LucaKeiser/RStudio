...
=> copy-on-modify => R objects are unchangeable or immutable.
...
=> because we assign a element of type double to a list of type integer. This triggers the copy-on-modify. 
...
=> object.size() does not account for shared elements within lists... "Sizes of objects using a compact internal representation may be over-estimated."
...
# copy-on-modify => new address in memory for the first element. The second element still refers to the same address as a. So the combined size does not increase.
b[[1]][[1]] <- 10
obj_size(b)
obj_size(a, b)
...
# copy-on-modify => new address for the second element too! So no element points to the same address as a anymore (2 new and separated addresses in memory...)
b[[2]][[1]] <- 10
obj_size(b)
obj_size(a, b)
